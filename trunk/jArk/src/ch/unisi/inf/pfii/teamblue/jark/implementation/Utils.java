...
public abstract class Utils implements Constants {
	...
	public final static int[] getFieldPosition(final int x, final int y) {
		final int posy = (int) ((float) FIELD_ROWS / (float) FIELD_HEIGHT * y);
		final int posx = (int) ((float) FIELD_COLUMNS / (float) FIELD_WIDTH * x);
	...
	public final static int[] getPixelPosition(final int x, final int y) {
		final int posy = y * (int) ((float) FIELD_HEIGHT / (float) FIELD_ROWS);
		final int posx = x
				* (int) ((float) FIELD_WIDTH / (float) FIELD_COLUMNS);
		return new int[] { posx, posy };
	}

	/**
	 * Converts an integer into a specific brick. Used when loading levels from
	 * files.
	 * 
	 * @param i
	 *            block type (integer)
