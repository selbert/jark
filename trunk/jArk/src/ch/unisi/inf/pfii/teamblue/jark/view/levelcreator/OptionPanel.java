...
public class OptionPanel extends JPanel {
	...
	public OptionPanel(final JTabbedPane tabbedPane, final FieldPanel fieldPanel) {
		...
		class AcceptedDocument extends PlainDocument {
			...
			public AcceptedDocument(final JTextField parent,
			...
			private void checkAccepted() {
				boolean accepted;
				if (numerical) {
					accepted = (!randomBonus.isSelected() || parent.getText()
							.matches("[0-9]{1,2}"));
				} else {
					accepted = (parent.getText().matches(
							"[a-zA-Z_][a-zA-Z_0-9]*") && parent.getText()
							.length() < 13);
				}
				if (accepted) {
		...
		gbcInner.gridx = 0;
		gbcInner.gridy = 0;
		final JCheckBox overrideCheck = new JCheckBox("Enable bricks override",
				true);
		overrideCheck
				.setToolTipText("<html>When enabled bricks in the editor will<br>be replaced with the selected one</html>");
		...
		randomBonus
				.setToolTipText("<html>Enable random bonus assignment at level start</html>");
		...
		numOfBonuses
				.setToolTipText("<html>Percentage of bonuses (bonus, malus and neutral) over the total number of bricks <br>that will be randomly assigned each time the level starts</html>");
